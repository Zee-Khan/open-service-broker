...
if [ "$#" -lt 1 ]; then
    echo "Usage: $(basename $0) osb_release_version [branch_to_release_from/develop] [branch_to_push_to/master]"
    exit 1
fi
...
./scripts/set-version.sh "$1-SNAPSHOT"
...
echo
echo OSB version bumped to "$1-SNAPSHOT" on $branch_to_release_from
